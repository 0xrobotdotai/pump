  ...
  const BaseInfo = () => (
    <div className="flex md:items-center md:justify-between w-full flex-col md:flex-row gap-4">
      <div className="flex md:items-center gap-1 lg:gap-4 text-md lg:text-2xl pt-2 text-white w-full md:w-2/3 line-clamp-2">
        ...
        {detail?.creator.toLowerCase() ===
        ROBOT_AI_CREATOR_ADDRESS.toLowerCase() ? (
          <Link href={`https://link.medium.com/0uZWZPhKePb`} target="_blank" title="Created by Ai Agent">
            <img src="/imgs/robot-ai-logo.png" alt="" className="w-4 h-4"></img>
          </Link>
        ) : null}
      </div>
      <div className="flex items-center gap-3 md:gap-5 text-white">
        ...
        <Button
          ...
          color="primary"
          onClick={() =>
            handleCopy(`https://pump.0xrobot.ai/token/${detail?.id}`)
          }
          key="copy"
        >
          Share
        ...
        <div className="flex gap-6 items-start">
          <CardMini title="Price">
            <span className="text-white">
              {helper.number.formatNumber(detail?.price || 0, "0.00000")} IOTX
            </span>
          </CardMini>
          <CardMini title="Market Cap">
            <span className="text-white">
              {" "}
              {Number(
                helper.number.formatNumber(detail?.marketCap || 0, "0.00")
              ).toLocaleString()}{" "}
              IOTX
            </span>
          </CardMini>
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          <div className="text-sm text-white/60 flex gap-2">
            ...
            <Link href={`/profile/${detail?.creator}`}>
              <span className="text-primary hover:text-primary underline">
                {helper.shortaddress(detail?.creator || "...")}
              </span>
            </Link>
          </div>
          <div className="flex items-center gap-2 text-sm">
            ...
            <Link
              ...
              target="blank"
              href={`https://iotexscan.io/token/${detail?.id}`}
            >
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <Link
              className="text-primary underline"
              target="blank"
              href={`https://iotexscan.io/token/${detail?.id}#token_holders`}
            >
              <Icon
  ...
  return (
    <div className="w-full pb-6 mt-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <Link
          className="block cursor-pointer text-white/50 font-semibold"
          href={"/"}
        >
          {"< Back"}
        </Link>
        <span className="text-white/60 text-xs">
          Disclaimer: Tokens listed on this platform are independent and not
          affiliated with RobotPump. Please DYOR.
        </span>
      </div>
      <div className="w-full flex flex-col lg:flex-row items-center justify-end gap-4 mb-4">
        <div className="text-white/50 text-sm">
          {detail?.launchedAt
            ? `Congratulations, Listed on MimoSwap`
            : "When 80% of tokens are sold out, and market cap reach 385,714.28 IOTX,  anyone can launch token."}
        </div>
        {detail?.launchedAt ? (
          <Button
            size="sm"
            as={Link}
            target="_blank"
            className="bg-[#acfe48] text-black"
            href={`https://mimo.exchange/swap?inputCurrency=IOTX&outputCurrency=${token?.detail?.id}`}
          >
            Go MimoSwap
          </Button>
        ) : (
          <Button
            size="sm"
            isDisabled={!detail?.completed}
            color="primary"
            onClick={() => token.launchToken.execute(parmas.mint as string)}
          >
            Launch Token
          </Button>
        )}
