...
const MemeCard = observer(() => {
  ...
  if (!token) return <LoadingCard />;

  return (
    <>
      {market.getRandomToken.loading.value ||
      !market.getRandomToken.value?.name ? (
        <LoadingCard />
      ) : (
        <Link
          href={`/token/${token?.id}`}
          className="flex h-[120px] lg:h-[160px] justify-start items-start cursor-pointer overflow-hidden rounded-xl border-2 border-primary/20 bg-gradient-to-r  to-[#92E7FF] from-[#FED25B] via-[#F3AEF5] hover:border-primary"
          key={token?.id}
        >
